...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/09/21 10:41:22 by keschouf          #+#    #+#             */
/*   Updated: 2026/09/21 10:41:23 by keschouf         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    *add_page(size_t size, size_t page_size, t_type type, char env_flag)
{
    ...
    while (page->next != NULL)
        page = page->next;
    new_page = init_zone(size, page_size, env_flag);
    if (!new_page)
        return (NULL);
...
void    *init_zone(size_t size, size_t page_size, char env_flag)
{
    ...
    if (env_flag & DEBUG_FLAG)
    {
        ft_putstr("[DEBUG] page addr = ");
        ft_print_hex((char *)page);
        ft_putstr("\n");
    }
    return (page);
}